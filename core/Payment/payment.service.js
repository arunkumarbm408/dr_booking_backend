...
class PaymentService {
  async createPayment(req, res) {
    try {
      ...
      if (appointment.paymentStatus === "Paid") {
        return res.status(400).json({ status: "fail", message: "Payment already submitted for this appointment" });
      }

      const existing = await Payment.findOne({ appointment: value.appointmentId });
      if (existing) {
        return res.status(400).json({ status: "fail", message: "Payment record already exists for this appointment" });
      }
      ...
      return res.status(201).json({
        status: "success",
        message: "Payment record created. Awaiting verification.",
        data: payment,
      });
    } catch (err) {
  ...
  async uploadScreenshot(req, res) {
    try {
      if (!req.file) {
        return res.status(400).json({ status: "fail", message: "No screenshot file uploaded" });
      }
